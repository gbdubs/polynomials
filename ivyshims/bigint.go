...
func setBigIntString(conf *Config, s string) (BigInt, error) {
...
func (i BigInt) toType(op string, conf *Config, which valueType) Value {
	switch which {
	case bigIntType:
		return i
	case bigRatType:
...
func mustFit(conf *Config, n int64) {
	max := conf.MaxBits()
